...
const translate = (model, locale, relation) => {
    ...
    if (!model[relation].find((v) => v.locale === locale)) {
                ...
                return class I18nQueryBuilder extends objectionModel.QueryBuilder {
                    ...
                    async patchTranslation(model, locale) {
                        ...
                        if (locale !== options.defaultLocale) { // Don't fetch the other translations on a default locale update
                            ...
                            model[options.relation] = (await getTranslations(this.modelClass().query(), model, this.modelClass().idColumn))[options.relation];
                        ...
                        return this.upsertGraphAndFetch(model).traverse(this.modelClass(), (m) => {
                            return translate(m, locale, options.relation);
                        });
                    }
                    ...
                    async deleteTranslation(id, locale) {
                        ...
                        const model = (await getTranslations(this.modelClass().query(), { [this.modelClass().idColumn] : id }, this.modelClass().idColumn));
